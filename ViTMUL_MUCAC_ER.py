...
from models_ViT.ViT_Unlearning_MUCAC_ER import ViT_unlearning_ER
import utils
...
def main(args):
    ...
    for seed in args.seeds:
        ...
        model = ViT_unlearning_ER(model_type=args.model_type, DASH_layer = args.DASH_layer)
        model = model.cuda()
        ...
        if args.training_type.lower() == 'cf3' and args.model_type == 'ViT-B-16':
            for name, param in model.named_parameters():
                if "visual_encoder.visual.transformer.resblocks.11." in name or "visual_encoder.visual.transformer.resblocks.10." in name or "visual_encoder.visual.transformer.resblocks.9." in name or name == 'fc.fc.weight' or name == 'fc.fc.bias' or 'visual_encoder.visual.ln_post' in name:
        ...
        elif args.training_type.lower() == 'cf3' and args.model_type == 'ViT-L-14':
            for name, param in model.named_parameters():
                if "visual_encoder.visual.transformer.resblocks.23." in name or "visual_encoder.visual.transformer.resblocks.22." in name or "visual_encoder.visual.transformer.resblocks.21." in name or name == 'fc.fc.weight' or name == 'fc.fc.bias' or 'visual_encoder.visual.ln_post' in name:
                    param.requires_grad = True
                else:
                    param.requires_grad = False
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, (x_, y_data) in enumerate(tqdm(used_data_loader)):
                y_ = torch.stack((y_data[0], y_data[2], y_data[3]), dim=1).type(torch.FloatTensor).cuda()

                outputs_ = model.forward_ER(x_.cuda())
                loss = criterion(outputs_, y_)
                overall_loss = loss
