...
#     show_images(imgs[7:19], label_strs, nrow=6)
    ...
    for seed in args.seeds:
        ...
        # args.model_type
        print('-'*77)
        if args.training_type.lower() in ['advneggrad']:
...
#         for name, param in model.named_parameters():
#             if ".visual.transformer.resblocks.6." in name or ".visual.transformer.resblocks.7." in name or ".visual.transformer.resblocks.8." in name:
#                  param.requires_grad = True
...
#             print(f'name: {name}, requires_grad: {param.requires_grad}')
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, (x_, y_) in enumerate(tqdm(used_data_loader, 
                ...
                if x_forget.size(0) != x_.size(0):
                    continue

                outputs_retain = model(x_.cuda())
                outputs_forget = model(x_forget.cuda())
                
#                 with torch.no_grad():
#                     outputs_teacher = teacher(x_.cuda())
                    
#                 loss_div_retain = criterion_div(outputs_retain, outputs_teacher)
                
                loss_retain = criterion(outputs_retain, y_.cuda())
