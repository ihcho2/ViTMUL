...
#     show_images(imgs[7:19], label_strs, nrow=6)
    ...
    for seed in args.seeds:
        ...
        # args.training_type
        print('-'*77)
        if args.training_type.lower() in ['aru']:
        ...
        forget_weight_net = deepcopy(model)
        forget_weight_net.train()
        criterion = torch.nn.CrossEntropyLoss()
        
        # Collecting gradient info
        for inputs, labels in forget_dataloader_test:
            inputs, labels = inputs.cuda(), labels.cuda()
            outputs = forget_weight_net(inputs)
            loss = criterion(outputs, labels)
            loss.backward()
        ...
        forget_noise_weight_net = deepcopy(model)
        forget_noise_weight_net.train()
        criterion = torch.nn.CrossEntropyLoss()
        attacker = PGD(model, attack_config)

        for inputs, labels in forget_dataloader_test:
            inputs, labels = inputs.cuda(), labels.cuda()
            adversarial_image = attacker(inputs, labels)
            noise_image = adversarial_image - inputs
            outputs_noise = forget_noise_weight_net(noise_image)
            loss_forget = criterion(outputs_noise, labels)
            loss_forget.backward()
        print("==========noise attacked!==========")
            
        amount_from_forget_set = args.pruning_ratio
        amount_from_retain_set = args.pruning_ratio
        
        weight_mask_dict = {}
        avg_pool = nn.AdaptiveAvgPool2d(output_size=(1, 1))
        
        for i, ((raw_name, raw_layer), (noise_name, noise_layer)) in enumerate(
                zip(forget_weight_net.named_modules(), forget_noise_weight_net.named_modules())):
            
            if 'visual_encoder.visual.' not in raw_name:
                continue
                
            if hasattr(raw_layer, "weight"):
                if 'conv' in raw_name:
                    cummulated_gradient = avg_pool(raw_layer.weight.grad.detach()).cpu().numpy().squeeze()
                    
                    cummulated_gradient = avg_pool(
                    raw_layer.weight.grad.detach() - noise_layer.weight.grad.detach()).cpu().numpy().squeeze()
                    distance_gradient = np.abs(cummulated_gradient)
                    k = int(distance_gradient.size*amount_from_forget_set)
                    idx = np.argpartition(distance_gradient.ravel(), k)[:k]
                    topk_idx = np.column_stack(np.unravel_index(idx, distance_gradient.shape))
                    m = torch.ones_like(raw_layer.weight)
                    m[topk_idx[:, 0], topk_idx[:, 1]] = 0
                    weight_mask_dict[raw_name] = m
                    
                else:
                    if 'resblocks.8' in raw_name or 'resblocks.9' in raw_name or 'resblocks.10' in raw_name or 'resblocks.11' in raw_name or 'ln_pre' in raw_name or 'ln_post' in raw_name:
                        print(raw_name)

                        if raw_layer.weight.dim() > 1:
                            cummulated_gradient = torch.mean(raw_layer.weight.grad.detach() - noise_layer.weight.grad.detach(), dim=-1).cpu().numpy().squeeze()
                        else:
                            cummulated_gradient = (raw_layer.weight.grad.detach() - noise_layer.weight.grad.detach()).cpu().numpy().squeeze()
        ...
        del forget_weight_net
        del forget_noise_weight_net
        
            
        model_reinit = deepcopy(model)
        for name, layer in model_reinit.named_modules():
            if name in weight_mask_dict.keys():
                w = layer.weight
                m = weight_mask_dict[name]
                layer.weight = nn.Parameter(w*m).cuda()
                
        # args.optimizer
        print('-'*77)
        if args.optimizer.lower() == 'sgd':
            if args.momentum != None:
                optimizer = optim.SGD(model_reinit.parameters(), lr=args.learning_rate, momentum=args.momentum)
                print(f'Using optimizer: {args.optimizer.lower()} / momentum: {args.momentum}')
            else:
                optimizer = optim.SGD(model_reinit.parameters(), lr=args.learning_rate)
                print(f'Using optimizer: {args.optimizer.lower()} / momentum: {args.momentum}')
        elif args.optimizer.lower() == 'adam':
            config_optimizer = {'opt': 'adam', 'lr': args.learning_rate, 'weight_decay': 0.02} 
            arg_opt = utils.AttrDict(config_optimizer)
            optimizer = create_optimizer(arg_opt, model_reinit)
        ...
        if args.training_type.lower() == 'cf3':
            for name, param in model_reinit.named_parameters():
...
#             print(f'name: {name}, requires_grad: {param.requires_grad}')
        ...
        print('='*77)
        print('Checking zero-shot unlearning performance')
        test_acc = evaluation(model_reinit, test_dataloader, criterion)
        unseen_acc = evaluation(model_reinit, unseen_dataloader, criterion)
        forget_acc = evaluation(model_reinit, forget_dataloader_test, criterion)
        mia = cal_mia(model_reinit.cuda(), forget_dataloader_test, unseen_dataloader)
        print('-'*77)
        ...
        for epoch in range(num_epochs):
            ...
            if test_acc['Acc'] > best_test_acc:
                ...
                if args.training_type.lower() == 'teacher' and args.save_teacher == True:
                    torch.save(model_reinit.state_dict(), f'MUFAC_{args.model_type}_teacher_seed_{seed}.pth')
