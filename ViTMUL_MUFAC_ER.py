...
from models_ViT.ViT_Unlearning_ER import ViT_unlearning_ER
import utils
    ...
    for seed in args.seeds:
        ...
        if args.training_type.lower() == 'cf3':
            for name, param in model.named_parameters():
                if "visual_encoder.visual.transformer.resblocks.11." in name or "visual_encoder.visual.transformer.resblocks.10." in name or "visual_encoder.visual.transformer.resblocks.9." in name or "visual_encoder.visual.transformer.resblocks.8." in name or "visual_encoder.visual.transformer.resblocks.7." in name or "visual_encoder.visual.transformer.resblocks.6." in name or "visual_encoder.visual.transformer.resblocks.5." in name or "visual_encoder.visual.transformer.resblocks.4." in name or"visual_encoder.visual.transformer.resblocks.3." in name or name == 'fc.weight' or name == 'fc.bias' or 'visual_encoder.visual.ln_post' in name:
        ...
        for epoch in range(num_epochs):
            ...
            if test_acc['Acc'] > best_test_acc:
                ...
                if args.training_type.lower() == 'teacher' and args.save_teacher == True:
                    torch.save(model.state_dict(), f'_ER_MUFAC_{args.model_type}_teacher_seed_{seed}.pth')
