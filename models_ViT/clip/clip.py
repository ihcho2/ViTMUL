...
from .model_moe import build_model_moe
from .model_ER import build_model_ER
...
def load_moe(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu",
         jit: bool = False, download_root: str = None):
    if name in _MODELS:
        model_path = _download(_MODELS[name], download_root or os.path.expanduser("~/.cache/clip"))
    elif os.path.isfile(name):
    ...
    try:
        ...
        if jit:
            ...
            jit = False
        state_dict = torch.load(model_path, map_location="cpu")
        model = build_model_moe(state_dict)#.to(device)
