...
class VisualTransformer_ER(nn.Module):
    ...
    def forward_ER(self, x: torch.Tensor, skip_last_layer=False, text_embedding=None, text_mask=None, use_checkpoint=True):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat([self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x], dim=1)  # shape = [*, grid ** 2 + 1, width]
        ...
        x = self.transformer.forward_ER(x, use_checkpoint=use_checkpoint)
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            self.visual = VisualTransformer_ER(
                input_resolution=image_resolution,
