...
def create_two_optimizer_freeze(args, model, filter_bias_and_bn=True):
    no_decay = ["bias", "LayerNorm.weight"]
    freeze_param_list=['positional_embedding',
               'text_projection',
               'logit_scale',
               ...
               'visual.transformer.MoE_layer',
               'visual.transformer.MoE_routing_layer',
               ]
    
    params_1 = []
    print('='*77)
    print('params excluded in params_1')
    for name, param in model.named_parameters():
        if ((not any(nd in name for nd in no_decay)) and ("visual_encoder" not in name)):
            exclude = 0
            for key in freeze_param_list:
                if key in name:
                    exclude = 1
                    break
            if not exclude:
    ...
    params_2 = []
    print('='*77)
    print('params excluded in params_2')
    for name, param in model.named_parameters():
        if ((any(nd in name for nd in no_decay)) and ("visual_encoder" not in name)):
            exclude = 0
            for key in freeze_param_list:
                if key in name:
                    exclude = 1
    ...
    params_3 = []
    print('='*77)
    print('params excluded in params_3')
        ...
        if ((not any(nd in name for nd in no_decay)) and ("visual_encoder" not in name)):
            ...
            for key in freeze_param_list:
                if key in name:
                    ...
                    break
            if not exclude:
                params_3.append(param)
            else:
                print(name)
            
    params_4 = []
    print('='*77)
    print('params excluded in params_4')
    for name, param in model.visual_encoder.named_parameters():
        if ((any(nd in name for nd in no_decay)) and ("visual_encoder" not in name)):
            exclude = 0
            for key in freeze_param_list:
